...
for u in $STORE_MAINTENANCE_USERS; do
    ...
    if [ ! "$?" -eq "0" ] ; then echo "------- ! could not delete user $u login profile !" ; else echo "------- deleted user $u login profile" ; fi
...
echo "------- removing buckets configuration for project $PROJ..."
...
echo "------- ...buckets configuration for project $PROJ was removed."
